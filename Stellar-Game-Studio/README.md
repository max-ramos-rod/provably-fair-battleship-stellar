...
## ZK Battleship Demo Flow

This repository can be demonstrated end-to-end with `zk-battleship` + `zk-battleship-risc0`:

1. Start the game frontend:
```bash
cd Stellar-Game-Studio
bun run dev:game zk-battleship
```

2. In the UI, create a game (or Quickstart), submit both player moves, and note the `Session ID`.

3. Generate proof artifacts in the prover project:
```bash
...
4. Copy the full `proof-output.json` contents.

5. Back in the frontend reveal phase (`Submit ZK Match Proof`), paste the JSON and submit.

Expected result: `ZK proof submitted. Game settled on-chain.`

Notes:
- `public_output.session_id` must match the loaded game session in the UI.
- If you use `RISC0_DEV_MODE=1`, `seal_hex` is `null` (good for local testing, not production proof submission).
