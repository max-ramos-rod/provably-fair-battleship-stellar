...
export interface ZkProofPayload {
  winner: 1 | 2;
  totalMoves: number;
  boardHashP1Hex: string;
  boardHashP2Hex: string;
  journalHex: string;
  sealHex: string;
}
...
function normalizeSealHexForVerifier(sealHex: string): string {
  ...
  if (normalized.length === RISC0_SEAL_EXPECTED_BYTES * 2) {
    return normalized;
  }

  if (normalized.length === RISC0_SEAL_RAW_BYTES * 2) {
    return `${RISC0_SELECTOR_HEX}${normalized}`;
  }

  throw new Error(
    `Invalid seal length: expected ${RISC0_SEAL_RAW_BYTES} or ${RISC0_SEAL_EXPECTED_BYTES} bytes, got ${normalized.length / 2}`
  );
}
function hexToBuffer(hex: string, expectedLength?: number): Buffer {
  ...
  if (expectedLength !== undefined && buf.length !== expectedLength) {
    throw new Error(`Invalid hex length: expected ${expectedLength} bytes, got ${buf.length}`);
  }
...
export class ZkBattleshipService {
  ...
  async submitShot(
    ...
    if (shotCell < 1 || shotCell > 16) {
      throw new Error('Shot must be between cell 1 and 16');
    }
  ...
  async makeGuess(
    ...
    return this.submitShot(sessionId, playerAddress, guess, signer, authTtlMinutes);
  }

  /**
   * Submit zk proof result to settle game on-chain.
   * Expects values produced by zk-battleship-risc0/proof-output.json.
   */
  async submitResult(
    sessionId: number,
    ...
    const boardHashP2 = hexToBuffer(payload.boardHashP2Hex, 32);
    const journal = hexToBuffer(payload.journalHex);
    const seal = hexToBuffer(normalizeSealHexForVerifier(payload.sealHex));

    const tx = await client.submit_result({
      session_id: sessionId,
      submitter: submitterAddress,
      winner: payload.winner,
      total_moves: payload.totalMoves,
      board_hash_p1: boardHashP1,
      board_hash_p2: boardHashP2,
      journal,
      seal,
    }, DEFAULT_METHOD_OPTIONS);
  ...
  /**
   * Reveal the winner after both players have submitted shots
   */
  async revealWinner(
    sessionId: number,
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('Transaction failed!')) {
        ...
        throw new Error('Transaction failed - check if both players have submitted shots and the game is still active');
      }
